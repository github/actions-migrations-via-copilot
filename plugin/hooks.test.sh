...
# run_case <label> <payload-json> <hook-bash> <expect: deny|allow|context>
run_case() {
...
run_case "CLI  allow rm Jenkinsfile"         '{"toolName":"bash","toolArgs":"{\"command\":\"rm Jenkinsfile\"}"}' "$DESTRUCTIVE" allow
...
run_case "VSC  allow rm Jenkinsfile w/redir" '{"tool_name":"run_in_terminal","tool_input":{"command":"rm Jenkinsfile 2>&1","mode":"sync"}}' "$DESTRUCTIVE" allow
...
if [ "$AFTER" -gt "$BEFORE" ]; then
  PASS=$((PASS+1)); echo "  ok   VSC  Stop appends scorecard entry"
else
  FAIL=$((FAIL+1)); echo "  FAIL VSC  Stop appends scorecard entry"
fi
