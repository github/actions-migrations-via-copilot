...
### 3. Separate Secrets by Sensitivity
- **Organization Secrets**: Shared infrastructure credentials (Docker Hub, NPM registry)
- **Repository Secrets**: Project-specific credentials
- **Environment Secrets**: Deployment-specific credentials (dev, staging, production)

### 4. Rotate Secrets After Migration
- Document which secrets were migrated
- Update all credentials after successful migration
- Remove old secrets from CircleCI after verification

## Migration Checklist

### Pre-Migration
- [ ] Audit all CircleCI contexts and their usage
- [ ] Identify which context variables are sensitive (secrets) vs. configuration (variables)
- [ ] Document which contexts are used by which jobs
- [ ] Identify all project environment variables
- [ ] Map CircleCI system variables to GitHub equivalents

### During Migration
- [ ] Create GitHub Secrets for sensitive context variables
- [ ] Create GitHub Variables for non-sensitive context variables
- [ ] Use organization-level secrets/variables for shared contexts
- [ ] Set up environment-specific secrets/variables for deployment contexts
- [ ] Update workflow files to reference new secrets/variables
- [ ] Replace CircleCI built-in variables with GitHub context equivalents
...
- [ ] Remove CircleCI contexts after successful migration
