...
### Template Expansion
- Always expand Azure DevOps templates inline in the resulting GitHub Actions workflow
- Replace template parameters with their actual values in the expanded code
- Document the source template in comments for future reference

### Variable Migration Strategy
- **Sensitive data** (passwords, tokens, connection strings) → GitHub Secrets
- **Configuration data** (endpoints, settings, flags) → GitHub Variables
- **Build metadata** (build numbers, version info) → GitHub context variables
- **Environment-specific** configuration → Environment variables in deployment environments

### Action Selection
- Always use latest stable versions of GitHub Actions
- Prefer verified creators from GitHub Marketplace
- Document action version choices in migration notes
- Use semantic versioning tags (e.g., `@v4`) for stability

### Security Improvements
- Leverage GitHub's built-in secret scanning
- Use environment protection rules for sensitive deployments
- Implement OIDC for cloud provider authentication
- Apply principle of least privilege to GitHub token permissions

## Common Migration Patterns

### Pattern 1: Build, Test, Deploy
Convert three-stage Azure DevOps pipeline to GitHub Actions jobs with dependencies.

### Pattern 2: Multi-Repository Pipelines
Use `repository` resources in workflows with proper checkout actions.

### Pattern 3: Approval Gates
Convert Azure DevOps manual validation to GitHub environment protection rules.

### Pattern 4: Scheduled Triggers
Convert Azure DevOps scheduled triggers to GitHub Actions cron schedules.
...
- [Azure DevOps to GitHub Actions Migration Guide](https://docs.github.com/en/actions/migrating-to-github-actions/migrating-from-azure-pipelines-to-github-actions)
