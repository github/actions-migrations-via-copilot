...
### Orb Expansion
- Always expand CircleCI orbs inline in the resulting GitHub Actions workflow
- Replace orb parameters with their actual values in the expanded code
- Document the source orb in comments for future reference

### Context Migration Strategy
- **Sensitive data** (passwords, tokens, API keys) → GitHub Secrets
- **Configuration data** (endpoints, settings, feature flags) → GitHub Variables
- **Build metadata** (build numbers, commit info) → GitHub context variables
- **Environment-specific** configuration → Environment-scoped secrets/variables
...
### Resource Class Considerations
- CircleCI resource classes don't have direct GitHub equivalents
- Use appropriate runner types (standard, large, etc.) based on needs
- Consider self-hosted runners for specific resource requirements

## Common Migration Patterns

### Pattern 1: Build, Test, Deploy
Convert three-job CircleCI workflow to GitHub Actions jobs with dependencies.

### Pattern 2: Multi-Executor Jobs
Use matrix strategy or separate jobs for different execution environments.

### Pattern 3: Approval Gates
Convert CircleCI approval jobs to GitHub environment protection rules.
...
- [CircleCI to GitHub Actions Migration Guide](https://docs.github.com/en/actions/migrating-to-github-actions/migrating-from-circleci-to-github-actions)
